...
public class FinanceManage {
    ...
    public FinanceEntity receiveFinanceByCaseID(int caseID) {
    ...
    public int receiveNextFinanceID() {
        int result = 0;
        Session session = sessionFactory.openSession();
        Transaction tx = null;
        try {
            tx = session.beginTransaction();
            Criteria cr = session.createCriteria(FinanceEntity.class);
...
//        financeEntity.setSigningcontractbycustomer9fi(financeBean.getSigningContractByCustomer9FileUpload());
        ...
        try {
            ...
            if (financeEntity.getCustomerId() == -1) {
                financeEntity.setId(new BigDecimal(receiveNextFinanceID()));
